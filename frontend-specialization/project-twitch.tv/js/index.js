// Hard-Coded Response JSON due to change in Twitch API 
var jsonResponse = [
  {
    "mature": false,
...
$(document).ready(function(){
  ...
  // api call - now replaced by jsonResponse
  jsonResponse.map(function(channel, index){
    if(channel.status == 404){
      var $main_card = $("<div>", {"class": "card"});
      $main_card.html(channel.message);
      $('.offline').append($main_card);
    }
    else{
      var $main_card = $("<div>", {"class": "card"});
      var $main_card_picture = $("<div>", {"class": "card-picture"}).css({
        'background-image' : 'url(' + channel.profile_banner + ')',
      })
      var $main_card_text = $("<div>", {"class": "card-text"}).html(channel.status);
      $main_card.append($main_card_picture);
      $main_card.append($main_card_text);
      $main_card.click(function(){
        window.open(channel.url, target='_blank');
      })
      
      $('.online').append($main_card);
    }
